...
token = "361169019:AAEPq2MPOFB7PHw-iSvR-pJTeHFROXDHSyM"
...
def all(call):
	if call.data:
		target = '-' + str(call.data.split('-')[1])
		day = int(redis.ttl("muteall:"+str(target)) / 86400)
		hours = int(day / 3600)
		min = int(hours / 60)
		sec = int(min / 60)
		if not redis.get("muteall:"+str(target)) or redis.ttl("muteall:"+str(target)) == -1:
			return "🔉"
		elif redis.ttl("muteall:"+str(target)) > 1 and redis.ttl("muteall:"+str(target)) < 60:
			return "00:00:{}".format(sec)
		elif redis.ttl("muteall:"+str(target)) > 60 and redis.ttl("muteall:"+str(target)) < 3600:
			return "00:{}:{}".format(min,sec)
		elif redis.ttl("muteall:"+str(target)) > 3600 and redis.ttl("muteall:"+str(target)) < 86400:
			return "{}:{}:{}".format(hours,min,sec)
		elif redis.ttl("muteall:"+str(target)) > 86400:
			return "{} Days And {}:{}:{}".format(day,hours,min,sec)
...
def inline(call):
	...
	if call.data:
		...
		mute_all = redis.ttl("muteall:"+str(target))
		...
		if str(call.from_user.id) in mods or str(call.from_user.id) in owner:
			...
			if call.data == "option"+str(target):
				if not lang:
					...
					l = types.InlineKeyboardButton("📢 Mutelist", callback_data='mutelist'+str(target))
					markup.add(k,l)
					...
					l = types.InlineKeyboardButton("📢 ليست بيصدا ها", callback_data='mutelist'+str(target))
					markup.add(k,l)
			...
			if call.data == "about"+str(target):
				if not lang:
					...
					markup.add(h)
					bot.edit_message_text("` توسعه دهندگان ربات هلپر بيوند ريبورن نسخه 1.0`", inline_message_id = call.inline_message_id, reply_markup=markup, parse_mode='Markdown')
		else:
			if not lang:
