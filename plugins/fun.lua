			...
			function tophoto_cb(arg,data)
				if data.content_.sticker_ then
					...
					if file_exi(tostring(name), tostring(apath), tostring(pasvand)) then
						...
						tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, pfile, "@kiavair", dl_cb, nil)
					else
			...
			function tosticker_cb(arg,data)
				if data.content_.ID == 'MessagePhoto' then
					...
					if file_exi(file..'_(1).jpg', tcpath..'/data/photo', 'jpg') then
						...
						tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, pfile, '@kiavair', dl_cb, nil)
					else
...
--------------------------------
	if matches[1]:lower() == "photo" then 
		...
		tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, "@kiavair", dl_cb, nil)
	end
...
return {               
	patterns = {
_config.cmd ..  "([Hh]elpfun)$",
_config.cmd ..  "([Ww]eather) (.*)$",
_config.cmd ..	"([Cc]alc) (.*)$",
_config.cmd ..	"([Tt]ime)$",
_config.cmd ..	"([Tt]ophoto)$",
_config.cmd ..	"([Tt]osticker)$",
_config.cmd ..	"([Vv]oice) +(.*)$",
_config.cmd ..	"([Pp]raytime) (.*)$",
_config.cmd ..	"([Pp]raytime)$",
_config.cmd ..	"([Aa]zan) (.*)$",
_config.cmd ..	"([Aa]zan)$",
_config.cmd ..	"([Tt]r) ([^%s]+) (.*)$",
